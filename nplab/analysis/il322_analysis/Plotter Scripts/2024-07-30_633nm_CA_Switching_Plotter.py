...
def plot_timescan(particle, save = False):
    ...
    if save == True:
        ...
        fig3.savefig(save_dir + particle.name + '633nm SERS Timescan' + '.svg', format = 'svg')
        plt.close(fig)
...
#%% Plotting of SERS with CA

def plot_timescan_ca(particle, save = False):
...
def plot_timescan_background_ca(particle, save = False):
...
#%% Loop over all particles and plot & save timescan_ca

print('\nPlotting timescans...')
for particle in tqdm(particles, leave = True):
    
    plot_timescan_ca(particle, save = False)
    plot_timescan_background_ca(particle, save = False)
...
#%% Peak fit for triple Gaussian region - 1300-1390 block
...
#%% Peak fit for triple Gaussian region - 1500-1590 block
...
for i, voltage in enumerate(voltages):
    ...
    ## Set up avg timescan
    keys = list(particles[i*3].h5_address.keys())
    keys = natsort.natsorted(keys)
    for key in keys:
        if 'SERS' in key:
            timescan = particles[i*3].h5_address[key]
